            ...
            {foreach from=$availableOptions item=availableOption}
                <li class="sortableNode">
                    <span class="sortableNodeLabel">
                        <label><input type="checkbox" name="values[{$option->optionName}][]" value="{$availableOption}"{if $availableOption|in_array:$value} checked{/if}> {lang}wcf.user.distinguished.{$availableOption}{/lang}</label>
                    </span>
                </li>
            {/foreach}
        </ol>
